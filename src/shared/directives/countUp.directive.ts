...
@Directive({
  selector: '[countUp]',
  providers: [Destroy, ShortNumberPipe],
})
export class CountUpDirective implements OnInit {
  private readonly count$ = new BehaviorSubject(0);
  private readonly duration$ = new BehaviorSubject(1000);
  private readonly short$ = new BehaviorSubject(false);
    ...
    this.currentCount$
      ...
      .subscribe((currentCount) => {
        this.renderer.setProperty(
          ...
          'innerHTML',
          !this.short ? new ShortNumberPipe().transform(currentCount) : currentCount
        );
      });
  }
}
